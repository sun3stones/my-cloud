...
public class WXService {
	...
	public String openId(HttpServletRequest request , HttpServletResponse response) throws IOException{
		...
		if(StringUtils.isBlank(code)){//code为空则微信授权，如果用户同意授权，页面将跳转至 redirect_uri/?code=CODE&state=STATE
			String uri= WxConfig.WX_MY()+request.getRequestURI().replace("/front", "");//授权后重定向地址
			System.out.println("重定向回："+uri);
			String url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid="+WxConfig.APPID()+"&redirect_uri="+uri+"&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect";
	...
	public void wxJsConfig(HttpServletRequest request){
		request.setAttribute("appId", WxConfig.APPID());
